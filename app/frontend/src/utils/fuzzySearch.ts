...
/**
 * Filters and scores items based on fuzzy matching.
 *
 * @param items - Array of items to search
 * @param query - Search query string
 * @param getSearchableFields - Function that returns an array of strings to search for each item
...
export const fuzzyFilter = <T>(
  ...
  return items
    .map(item => {
      ...
      const score = Math.max(...fields.map(field => fuzzyScore(field, lowerQuery)));
      return { item, score };
    })
